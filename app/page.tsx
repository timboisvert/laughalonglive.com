  ...
  return (
    <>
      <section className="space-y-8 py-6">
        <h2 className="text-5xl sm:text-6xl md:text-7xl font-bold">Upcoming Shows:</h2>
      ...
      <section className="space-y-8 py-6">
        <h2 className="text-5xl sm:text-6xl md:text-7xl font-bold">Past Shows:</h2>
        ...
        >
          {pastshows.map((show) => (
            <Link key={show.id} href={`/pastshows/${show.id}`} className="flex flex-col items-center text-center group">
              <div className="relative w-96 h-[36rem] overflow-hidden rounded-lg border-2 border-black shadow-md transition-transform group-hover:scale-105">
                <Image
                  src={show.posterSrc || "/placeholder.svg"}
                  alt={`${show.movie} Poster Thumbnail`}
